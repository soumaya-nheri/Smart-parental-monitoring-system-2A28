...
void activite::afficher_activite(QTableWidget *t)
{
    ...
    int i;
    int j=0;
    qry.exec("select a.*,en.* from activite a inner join enfants en on a.id_enfant=en.id_enfant");
    while(qry.next()){
        j++;
    }
    t->setRowCount(j);
...
qDebug()<<"row int"+text_row_int;
qDebug()<<"row text"+text_row;
...
void activite::rechercher_activite(QTableWidget *t,QString arg1)
{
    QSqlQuery query;
       int i;
       int j=0;
       query.exec("SELECT a.*,en.* from activite a inner join enfants en on a.id_enfant=en.id_enfant where a.ID_ACTIVITE like'%"+arg1+"%' or a.NOM_ACTIVITE like'%"+arg1+"%' or a.DESCRIPTION like'%"+arg1+"%'or a.DUREE like'%"+arg1+"%' or a.TEMPS like'%"+arg1+"%' or a.DATEE like'%"+arg1+"%' or a.RDV like'%"+arg1+"%' or en.NOM_ENFANT like'%"+arg1+"%'");
...
void activite::trier_activite(QTableWidget *t)
{
    ...
    int i;
    int j=0;
    qry.exec("select a.*,en.* from activite a inner join enfants en on a.id_enfant=en.id_enfant order by CAST(SUBSTR(a.datee,7,4) AS NUMBER) ASC");
    while(qry.next()){
        j++;
    }
    t->setRowCount(j);
